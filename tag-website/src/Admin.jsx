...
function Admin() {
  ...
  const sqlTemplates = {
    ...
    '📊 Analytics': [
      ...
      { name: 'Abandoned carts (all time)', query: "SELECT a.session_id, a.created_at, a.event_data::json->>'dropoff_date' AS dropoff_date, a.event_data::json->>'pickup_date' AS pickup_date, a.event_data::json->>'days_parking' AS days, a.event_data::json->>'departure_destination' AS destination, a.event_data::json->>'departure_airline' AS airline, a.event_data::json->>'departure_time' AS flight_time FROM audit_logs a WHERE a.event = 'flight_selected' AND a.session_id NOT IN (SELECT DISTINCT session_id FROM audit_logs WHERE event IN ('payment_succeeded', 'booking_confirmed') AND session_id IS NOT NULL) AND a.session_id IS NOT NULL ORDER BY a.created_at DESC LIMIT 500", note: 'All abandoned with flight details (max 500)' },
      { name: 'Abandoned carts (recent 50)', query: "SELECT a.session_id, a.created_at, a.event_data::json->>'dropoff_date' AS dropoff_date, a.event_data::json->>'pickup_date' AS pickup_date, a.event_data::json->>'days_parking' AS days, a.event_data::json->>'departure_destination' AS destination, a.event_data::json->>'departure_airline' AS airline, a.event_data::json->>'departure_time' AS flight_time FROM audit_logs a WHERE a.event = 'flight_selected' AND a.session_id NOT IN (SELECT DISTINCT session_id FROM audit_logs WHERE event IN ('payment_succeeded', 'booking_confirmed') AND session_id IS NOT NULL) AND a.session_id IS NOT NULL ORDER BY a.created_at DESC LIMIT 50", note: 'Last 50 abandoned' },
      { name: 'Abandoned carts (today)', query: "SELECT a.session_id, a.created_at, a.event_data::json->>'dropoff_date' AS dropoff_date, a.event_data::json->>'pickup_date' AS pickup_date, a.event_data::json->>'days_parking' AS days, a.event_data::json->>'departure_destination' AS destination, a.event_data::json->>'departure_airline' AS airline, a.event_data::json->>'departure_time' AS flight_time FROM audit_logs a WHERE a.event = 'flight_selected' AND DATE(a.created_at) = CURRENT_DATE AND a.session_id NOT IN (SELECT DISTINCT session_id FROM audit_logs WHERE event IN ('payment_succeeded', 'booking_confirmed') AND session_id IS NOT NULL) AND a.session_id IS NOT NULL ORDER BY a.created_at DESC", note: 'Today only' },

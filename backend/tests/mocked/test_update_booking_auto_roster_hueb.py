...
def captured(monkeypatch):
    """Capture the BackgroundTask schedule calls.

    Both `fire_engine_async` (shadow planner) and `auto_create_or_extend_async`
    (live auto-roster) get patched to record their invocation. The test
    assertions then check which ones the endpoint fired.
    """
    state = {
